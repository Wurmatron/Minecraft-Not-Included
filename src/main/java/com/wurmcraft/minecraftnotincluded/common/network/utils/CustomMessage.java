package com.wurmcraft.minecraftnotincluded.common.network.utils;
...
public abstract class CustomMessage<T extends CustomMessage<T>>
  ...
  private static final <T extends CustomMessage<T>> void checkAndHandle(
    ...
    IThreadListener thread = MinecraftNotIncluded.proxy.getThreadListener(ctx);
    thread.addScheduledTask(() -> msg.process(MinecraftNotIncluded.proxy.getPlayer(ctx), ctx.side));
  }
  ...
  public final IMessage onMessage(T msg, MessageContext ctx) {
    if (!msg.isValidOnSide(ctx.side)) {
      ...
      msg.process(MinecraftNotIncluded.proxy.getPlayer(ctx), ctx.side);
    }
    return null;
  }
  ...
  public abstract static class CustomtServerMessage<T extends CustomMessage<T>>
      extends CustomMessage<T> {
