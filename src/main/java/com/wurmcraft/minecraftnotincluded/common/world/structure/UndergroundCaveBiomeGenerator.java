...
public class UndergroundCaveBiomeGenerator implements ICubicStructureGenerator {
     ...
     */
    private static final int LARGE_NODE_RARITY = 20;
     ...
     */
    private static final int BIG_CAVE_RARITY = 2;

    /**
     * Value added to the size of the cave (radius)
     */
    private static final double CAVE_SIZE_ADD = 6D;
     ...
     */
    private static final float FLATTEN_FACTOR = 0.2f;

    /**
     * If steeper cave is allowed - this value will be used instead of FLATTEN_FACTOR
     */
    private static final float STEEPER_FLATTEN_FACTOR = 0.4f;
     ...
     */
    private static final float MAX_ADD_DIRECTION_CHANGE_VERT = 1.5f;
    ...
    public void generate(World world, CubePrimer cubePrimer, CubePos cubePos) {
        generate(world, world.rand, cubePrimer,cubePos.getX(), cubePos.getY(), cubePos.getZ(), cubePos);
    }
    ...
    protected void generate(World world, Random rand, CubePrimer cube,
        ...
        for (int node = 0; node < nodes; ++node) {
            ...
            for (int branch = 0; branch < subBranches; ++branch) {
                ...
                float baseHorizSize = rand.nextInt(2) * 2.0F + rand.nextFloat() + 4f;

                if (rand.nextInt(BIG_CAVE_RARITY) == 0) {
                    baseHorizSize *= rand.nextInt(3) * rand.nextFloat() * 3.0F + 12F;
                }
    ...
    private void generateLargeNode(CubePrimer cube, Random rand, long seed, CubePos generatedCubePos,
        ...
        float baseHorizSize =( 6.0F + rand.nextFloat() * 6.0F) + 6f;
    ...
    private void generateNode(CubePrimer cube, long seed,
        ...
        if (maxWalkedDistance <= 0) {
            int maxBlockRadius = cubeToMinBlock(8 - 1);
